        ...
        def preprocess_obss(obss, device=None):
            ...
            for obs in obss:
                if isinstance(obs, tuple):
                    ...
                    texts.append(obs[0]["mission"])   
                elif isinstance(obs, dict):
                    images.append(obs["image"])
                    texts.append(obs["mission"])
                else:
                    raise TypeError("Unsupported data type in observations list.")
                    breakpoint()


            processed_images = preprocess_images(images, device=device)
            processed_texts = preprocess_texts(texts, vocab=vocab, device=device)
